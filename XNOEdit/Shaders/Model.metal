...
// Matches C# vertex layout: 56 bytes tightly packed
struct ModelVertex {
    packed_float3 position;  // 12 bytes @ 0
    packed_float3 normal;    // 12 bytes @ 12
    packed_float4 color;     // 16 bytes @ 24
    packed_float2 uv0;       //  8 bytes @ 40
    packed_float2 uv1;       //  8 bytes @ 48
};                           // Total: 56 bytes
    ...
    uint vid [[vertex_id]])
{
    ...
    float4 worldPos = pf->model * float4(pos, 1.0);
    out.world_position = worldPos.xyz;

    float3x3 normalMatrix = float3x3(pf->model[0].xyz, pf->model[1].xyz, pf->model[2].xyz);
    float3 normal = float3(v.normal);
    out.normal = normalize(normalMatrix * normal);

    float3x3 tbn = calculateTangentSpace(out.normal);
    out.tangent = tbn[0];
    out.bitangent = tbn[1];
